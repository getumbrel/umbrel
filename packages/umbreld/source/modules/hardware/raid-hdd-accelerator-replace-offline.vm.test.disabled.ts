...
describe('RAID HDD accelerator replacement with an offline device', () => {
	...
	test('detects HDD and SSD devices', async () => {
		...
		replacementAcceleratorDeviceId = ssds[2].id!
	})
	...
	test('adds a mirrored accelerator', async () => {
		await umbreld.client.hardware.raid.addAccelerator.mutate({
			deviceIds: [acceleratorDeviceId1, acceleratorDeviceId2],
		})
	})

	test('removes one accelerator SSD and boots back up degraded', async () => {
		await umbreld.vm.powerOff()
		await umbreld.vm.removeNvme({slot: 1})
		await umbreld.vm.powerOn()
		await umbreld.login()

		const status = await umbreld.client.hardware.raid.getStatus.query()
		expect(
			status.accelerator?.devices?.some((device) => device.id === acceleratorDeviceId1 && device.status !== 'ONLINE'),
		).toBe(true)
	})

	test('reports a non-zero accelerator size while degraded', async () => {
		const status = await umbreld.client.hardware.raid.getStatus.query()
		expect(status.accelerator).toMatchObject({exists: true})
		expectSizeToBeWithinTenPercentOfGb(status.accelerator?.l2arcSize, 10)
		expectSizeToBeWithinTenPercentOfGb(status.accelerator?.specialSize, expectedMirroredAcceleratorSpecialSizeGb)
	})

	test('replaces one accelerator SSD while it is offline', async () => {
	...
	test('waits for accelerator replacement to complete while it is offline', async () => {
		await pWaitFor(
			async () => {
				const status = await umbreld.client.hardware.raid.getStatus.query()
	...
	test('reports accelerator replace events while it is offline', () => {
	...
	test('reports accelerator replace status while it is offline', () => {
	...
	test('reports the replacement accelerator device after the offline replace', async () => {
		const status = await umbreld.client.hardware.raid.getStatus.query()
		const acceleratorIds = status.accelerator!.devices!.map((device) => device.id).sort()
		expect(acceleratorIds).toEqual([acceleratorDeviceId2, replacementAcceleratorDeviceId].sort())
